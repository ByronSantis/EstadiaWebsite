...
if(isset($_POST['update'])){
   ...
   $update_listing = $conn->prepare("UPDATE `property` SET property_name = ?, address = ?, price = ?, type = ?, offer = ?, status = ?, furnished = ?, bedroom = ?, bathroom = ?, total_floors = ?, room_floor = ?, lift = ?, security_guard = ?, play_ground = ?, garden = ?, water_supply = ?, power_backup = ?, parking_area = ?, gym = ?, shopping_mall = ?, hospital = ?, school = ?, market_area = ?, description = ? WHERE id = ?");   
   $update_listing->execute([$property_name, $address, $price, $type, $offer, $status, $furnished, $bedroom, $bathroom, $total_floors, $room_floor, $lift, $security_guard, $play_ground, $garden, $water_supply, $power_backup, $parking_area, $gym, $shopping_mall, $hospital, $school, $market_area, $description, $update_id]);

   $success_msg[] = 'Publicacion actualizada';
...
<section class="property-form">
   ...
   <form action="" method="POST" enctype="multipart/form-data">
      ...
      <h3>Actualizar detalles de la vivienda</h3>
      <div class="box">
         <p>Nombre vivienda<span>*</span></p>
         <input type="text" name="property_name" required maxlength="50" placeholder="Actualiza el nombre de la vivienda" value="<?= $fetch_property['property_name']; ?>" class="input">
      </div>
      <div class="flex">
         <div class="box">
         ...
         <div class="box">
            ...
            <input type="text" name="address" required maxlength="100" placeholder="Actualiza la direccion de la vivienda" class="input" value="<?= $fetch_property['address']; ?>">
         </div>
         <div class="box">
         ...
         <div class="box">
            ...
            <select name="bedroom" required class="input">
               <option value="<?= $fetch_property['bedroom']; ?>" selected><?= $fetch_property['bedroom']; ?> Dormitorio</option>
         ...
         <div class="box">
            ...
            <select name="bathroom" required class="input">
               <option value="<?= $fetch_property['bathroom']; ?>" selected><?= $fetch_property['bathroom']; ?> Baño</option>
         ...
         <div class="box">
            ...
            <select name="balcony" required class="input">
               <option value="<?= $fetch_property['balcony']; ?>" selected><?= $fetch_property['balcony']; ?> Balcon</option>
         ...
         <div class="box">
            ...
            <input type="number" name="total_floors" required min="0" max="99" maxlength="2" placeholder="Pisos de la vivienda" class="input" value="<?= $fetch_property['total_floors']; ?>">
         </div>
         <div class="box">
            <p>N de casa o departamento<span>*</span></p>
            <input type="number" name="room_floor" required min="0" max="9999999" maxlength="7" placeholder="Numero de la vivienda" class="input" value="<?= $fetch_property['room_floor']; ?>">
         </div>
      </div>
      <div class="box">
         <p>Descripcion<span>*</span></p>
         <textarea name="description" maxlength="1000" class="input" required cols="30" rows="10" placeholder="Agrega una descripcion de tu vivienda" ><?= $fetch_property['description']; ?></textarea>
      </div>
      <div class="checkbox">
         <div class="box">
            <p><input type="checkbox" name="lift" value="yes" <?php if($fetch_property['lift'] == 'yes'){echo 'checked'; } ?> />Ascensores</p>
            <p><input type="checkbox" name="security_guard" value="yes" <?php if($fetch_property['security_guard'] == 'yes'){echo 'checked'; } ?> />Camara de seguridad</p>
            <p><input type="checkbox" name="play_ground" value="yes" <?php if($fetch_property['play_ground'] == 'yes'){echo 'checked'; } ?> />Plaza de juegos</p>
            <p><input type="checkbox" name="garden" value="yes" <?php if($fetch_property['garden'] == 'yes'){echo 'checked'; } ?> />Jardin delantero y trasero</p>
            <p><input type="checkbox" name="water_supply" value="yes" <?php if($fetch_property['water_supply'] == 'yes'){echo 'checked'; } ?> />Agua</p>
            <p><input type="checkbox" name="power_backup" value="yes" <?php if($fetch_property['power_backup'] == 'yes'){echo 'checked'; } ?> />Electricidad</p>
         </div>
         <div class="box">
            <p><input type="checkbox" name="parking_area" value="yes" <?php if($fetch_property['parking_area'] == 'yes'){echo 'checked'; } ?> />Estacionamiento</p>
            <p><input type="checkbox" name="gym" value="yes" <?php if($fetch_property['gym'] == 'yes'){echo 'checked'; } ?> />Gimnasio</p>
            <p><input type="checkbox" name="shopping_mall" value="yes" <?php if($fetch_property['shopping_mall'] == 'yes'){echo 'checked'; } ?> />Mall</p>
            <p><input type="checkbox" name="hospital" value="yes" <?php if($fetch_property['hospital'] == 'yes'){echo 'checked'; } ?> />Hospital</p>
            <p><input type="checkbox" name="school" value="yes" <?php if($fetch_property['school'] == 'yes'){echo 'checked'; } ?> />Escuela</p>
            <p><input type="checkbox" name="market_area" value="yes" <?php if($fetch_property['market_area'] == 'yes'){echo 'checked'; } ?> />Super y/o minimarket</p>
         </div>
      </div>
      <div class="box">
         <img src="uploaded_files/<?= $fetch_property['image_01']; ?>" class="image" alt="">
         <p>Actualizar imagen 1</p>
      ...
      <div class="flex"> 
         ...
         <div class="box">
            ...
            <?php } ?>
            <p>Actualizar imagen 5</p>
            <input type="file" name="image_05" class="input" accept="image/*">
         </div>   
      </div>
      <input type="submit" value="Actualizar detalles" class="btn" name="update">
   </form>
   <?php
      }
   }else{
