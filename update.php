<?php
    ...
    if(isset($_POST['submit'])){
        ...
        if(!empty($number)){
            ...
            $success_msg[] = 'Numero actualizado correctamente';
        }
        ...
        if(!empty($email)){
            ...
            if($verify_email->rowCount() > 0){
                $warning_msg[] = 'Email ya existente';
            }else{
                $update_email = $conn->prepare("UPDATE `users` SET email = ? WHERE id = ?");
                $update_email->execute([$email, $user_id]);
                $success_msg[] = 'Email actualizado correctamente';
            }      
        }  
...
<head>
    ...
    <title>EstadiaChile update</title>
...
<section class="form-container">
    ...
    <form action="" method="POST">
        ...
        placeholder="<?=$fetch_account['number'];?>" min="0" max="9999999999" class="box">
        <input type="password" name="old_pass"  maxlength="50"
        placeholder="Ingresa tu contraseña antigua" class="box">
        <input type="password" name="new_pass" maxlength="50"
        placeholder="Crea una nueva contraseña" class="box">
        <input type="password" name="c_pass" maxlength="50"
        placeholder="Confirma tu nueva contraseña" class="box">
